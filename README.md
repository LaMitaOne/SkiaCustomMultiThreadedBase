...
***Threaded rendering at all works and looks ok now, no flicker at all (except when we do more than 4 threads, then some artefacts but still running), cause we wait a short moment (except strip 1 only 300ns) after create each workerthread but we get memleak and fill up and die after a while...somebody see it maybe or has an idea?***
