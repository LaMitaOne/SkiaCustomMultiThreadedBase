...
***Threaded rendering at all works and looks ok now, no flicker at all (except when we do more than 4 threads, then some artifacts but still running even at 64), cause we wait a short moment (less than sleep(1)) after create each workerthread but we get memleak and fill up and die after a while...somebody see it maybe or has an idea?***     
    
On dual core no artifacts even at 64 threads, but getting a bit slow, still nice to test. Artifacts so probably timing at faster pc   
